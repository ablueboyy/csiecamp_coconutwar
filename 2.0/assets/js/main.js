/* =========================================================
 * COCONUT WARS 2.0 — 應用進入點（主控台 + 播放視窗）
 * ========================================================= */
...
const bus = new BroadcastChannel('coconut-wars-2');
...
async function handleSettle() {
  ...
  if (GAME.round >= GAME.numRounds) {
    ...
    GAME.round += 1;
    rollHarvest();          // 決定下一回合豐收島
    broadcastState();
    enterControl();
  }
}

function openDisplay() { window.open(location.pathname + '?view=display', 'cw2-display', 'width=1280,height=880'); }
